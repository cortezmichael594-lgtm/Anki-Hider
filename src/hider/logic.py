...
class HiderManager:
    ...
    def on_overview_did_refresh(self, overview: Overview) -> None:
        try:
            if not self._config.auto_start_study:
                return
            if mw.state == constants.REVIEW_STATE:
                # Al entrar a un mazo, Anki encola DOS refrescos del resumen: el de
                # Overview.show() y el que provoca el operation_did_execute de
                # seleccionar el mazo. Cada uno pinta la pagina en el callback de
                # su propio QueryOp sin comprobar el estado. Si nuestro salto al
                # repaso adelanta al segundo, su callback rezagado pinta el resumen
                # ENCIMA del repaso: pantalla pre-estudio con el boton de estudiar
                # muerto (el puente sigue siendo el del repasador) y el chrome
                # oculto. Ese pintado rezagado dispara este mismo hook, asi que se
                # repara reentrando en "review": _reviewCleanup no hace nada si el
                # destino es "review" y show() reinicia _reps, con lo que el
                # repasador se repinta completo y su pagina queda la ultima.
                mw.moveToState(constants.REVIEW_STATE)
                return
            ...
            def _has_due_card(col: Collection) -> bool:
                # counts() anuncia tarjetas de aprendizaje que vencen mas tarde,
                # asi que no sirve para decidir: entrariamos al repaso y nextCard,
                # al no obtener tarjeta de get_queued_cards, rebotaria al resumen.
                # Se pregunta a la misma fuente que usa el repasador (idempotente).
                return bool(col.sched.get_queued_cards(fetch_limit=1).cards)

            def _start_if_due(has_due: bool) -> None:
                if (
            ...
            QueryOp(parent=mw, op=_has_due_card, success=_start_if_due).failure(
